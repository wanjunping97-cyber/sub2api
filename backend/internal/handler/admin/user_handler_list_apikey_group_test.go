...
func TestAdminUserList_ParsesBalanceResetDue(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cases := []struct {
		name  string
		query string
		...
		{"overdue", "?balance_reset_due=overdue", service.BalanceResetDueOverdue},
		{"due soon", "?balance_reset_due=due_soon", service.BalanceResetDueSoon},
		{"missing", "", ""},
		{"unknown ignored", "?balance_reset_due=tomorrow", ""},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			stub := &listUsersFilterStub{AdminService: newStubAdminService()}
			...
			require.Equal(t, http.StatusOK, w.Code)
			require.Equal(t, tc.want, stub.captured.BalanceResetDue)
		})
	}
}
